...
export const getOptMail = async (
  email: string,
  contractId: string | undefined,
  type: string
) => {
  const response = await axios.get(
    `${BASE_URL}public/otp/${email}/${type}/${contractId}`
  );
...
export const verifySMSCode = async (data: any) => {
  const response = await axios.post(
    `${BASE_URL}public/otp/verify-for-sign-with-sms`,
    data
  );
  return response.data;
};
export const validatePhone = async (phone: any) => {
  const response = await axiosInstance.get(
    `contract/check-duplicate?tableName=party&columnName=phone&value=${phone}`
  );
  return response.data;
};
export const validateMail = async (email: any) => {
  const response = await axiosInstance.get(
    `contract/check-duplicate?tableName=party&columnName=email&value=${email}`
  );
  return response.data;
};
